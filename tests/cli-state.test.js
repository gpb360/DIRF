...
test("bounded active state compares an independent canonical checkpoint for the active attempt", () => {
  const home = freshHome();
  const main = mkdtempSync(join(tmpdir(), "bounded-same-attempt-canonical-"));
  execFileSync("git", ["init", "-q"], { cwd: main, timeout: TIMEOUT });
  execFileSync("git", ["config", "user.email", "dirf@example.invalid"], { cwd: main, timeout: TIMEOUT });
  execFileSync("git", ["config", "user.name", "DIRF Test"], { cwd: main, timeout: TIMEOUT });
  writeFileSync(join(main, "review.txt"), "A\n");
  ...
  run(["setup", main], { DIRF_HOME: home }, main);
  const activeAttempt = JSON.parse(run(["build", "active-review", "review PR 21", "--path", main, "--json"], { DIRF_HOME: home }, main)).attempt;
  run(["resume", activeAttempt.id, "--path", main], { DIRF_HOME: home }, main);
  run([
    "record-progress", "Stored active attempt at A", "--attempt", activeAttempt.id, "--path", main,
    "--next", "Merge stale A", "--work-item", "pr:21", "--review-revision", revisionA,
  ], { DIRF_HOME: home }, main);

  const canonical = [
    "# DIRF Handoff", "", "## Work item", "", "pr:21", "", "## Review revision", "", revisionB, "",
    "## Attempt ID", "", activeAttempt.id, "", "## Update number", "", "2", "",
    "## Exact next action", "", "Fix the current B review", "",
  ].join("\n");
  const canonicalFile = join(main, "canonical-b.md");
  writeFileSync(canonicalFile, canonical);
  ...
  const active = JSON.parse(run(["state", "active", "--path", main, "--json"], { DIRF_HOME: home }, main));
  const canonicalPath = join(home, "projects", active.project, "HANDOFF.md");
  assert.equal(active.attempt.needs_refresh, true);
  assert.equal(active.attempt.next_action, null);
  assert.equal(active.attempt.related_task_relation, "candidate_newer");
  assert.equal(active.attempt.related_attempt_id, activeAttempt.id);
  assert.equal(active.attempt.newer_attempt_id, activeAttempt.id);
  assert.equal(active.attempt.related_handoff_path, canonicalPath);
  assert.equal(active.attempt.newer_handoff_path, canonicalPath);
  assert.doesNotMatch(JSON.stringify(active), /Merge stale A/);
});

test("bounded active state compares a same-attempt canonical checkpoint without reading unrelated handoffs", () => {
  const home = freshHome();
  const main = mkdtempSync(join(tmpdir(), "bounded-no-reference-"));
  execFileSync("git", ["init", "-q"], { cwd: main, timeout: TIMEOUT });
  run(["setup", main], { DIRF_HOME: home }, main);
  const activeAttempt = JSON.parse(run([
    "build", "active-docs", "maintain local docs", "--path", main, "--json",
  ], { DIRF_HOME: home }, main)).attempt;
  run(["resume", activeAttempt.id, "--path", main], { DIRF_HOME: home }, main);
  const unrelated = JSON.parse(run([
    "build", "unrelated-history", "archive unrelated notes", "--path", main, "--json",
  ], { DIRF_HOME: home }, main));
  const unrelatedHandoff = join(dirname(unrelated.workflow), "HANDOFF.md");
  rmSync(unrelatedHandoff);
  mkdirSync(unrelatedHandoff);

  const canonical = [
    "# DIRF Handoff", "", "## Attempt ID", "", activeAttempt.id, "", "## Update number", "", "2", "",
    "## Exact next action", "", "Use the newer canonical checkpoint", "",
  ].join("\n");
  const canonicalFile = join(main, "canonical-no-reference.md");
  writeFileSync(canonicalFile, canonical);
  run(["state", "write-handoff", "--file", canonicalFile, "--path", main], { DIRF_HOME: home }, main);

  const active = JSON.parse(run(["state", "active", "--path", main, "--json"], { DIRF_HOME: home }, main));
  const canonicalPath = join(home, "projects", active.project, "HANDOFF.md");
  assert.equal(active.attempt.id, activeAttempt.id);
  assert.equal(active.attempt.needs_refresh, true);
  assert.equal(active.attempt.next_action, null);
  assert.equal(active.attempt.related_attempt_id, activeAttempt.id);
  assert.equal(active.attempt.related_handoff_path, canonicalPath);
...
test("record-progress rejects an explicit empty CLI work item without mutation", () => {
  const home = freshHome();
  const main = mkdtempSync(join(tmpdir(), "empty-cli-work-item-"));
  execFileSync("git", ["init", "-q"], { cwd: main, timeout: TIMEOUT });
  execFileSync("git", ["config", "user.email", "dirf@example.invalid"], { cwd: main, timeout: TIMEOUT });
  execFileSync("git", ["config", "user.name", "DIRF Test"], { cwd: main, timeout: TIMEOUT });
  writeFileSync(join(main, "review.txt"), "A\n");
  ...
  run(["setup", main], { DIRF_HOME: home }, main);
  const built = JSON.parse(run(["build", "review", "review PR 21", "--path", main, "--json"], { DIRF_HOME: home }, main));
  run([
    "record-progress", "Store revision B", "--attempt", built.attempt.id, "--path", main,
    "--next", "Fix PR 21", "--work-item", "pr:21", "--review-revision", revisionB,
  ], { DIRF_HOME: home }, main);
  const [project] = JSON.parse(run(["state", "list", "--json"], { DIRF_HOME: home }, main));
  const projectDir = join(home, "projects", project.slug);
  const canonicalPath = join(projectDir, "HANDOFF.md");
  const attemptPath = join(dirname(built.workflow), "HANDOFF.md");
  const sequencePath = join(projectDir, ".progress-sequence");
  const before = [canonicalPath, attemptPath, sequencePath].map((path) => readFileSync(path, "utf8"));

  const rejected = spawnSync(process.execPath, [CLI,
    "record-progress", "Reject empty identity", "--attempt", built.attempt.id, "--path", main,
    "--next", "Keep current B", "--work-item", "", "--review-revision", revisionB,
  ], { cwd: main, encoding: "utf8", timeout: TIMEOUT, env: { ...process.env, DIRF_HOME: home } });
  assert.notEqual(rejected.status, 0);
  assert.match(rejected.stderr, /not recorded.*invalid_work_item/i);
  assert.deepEqual([canonicalPath, attemptPath, sequencePath].map((path) => readFileSync(path, "utf8")), before);
});
